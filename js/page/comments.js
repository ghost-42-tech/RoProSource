            ...
            <div class="comment-form">
                <form class="form-horizontal ng-pristine ng-valid" role="form">
                    <div class="form-group">
                        <textarea id="messageBox" class="form-control input-field rbx-comment-input blur" placeholder="Write a comment!" rows="1"></textarea>
                    ...
                    <button id="postComment" type="button" class="btn-secondary-md rbx-post-comment">Post Comment</button>
                </form>
            </div>
...
async function generateComment(commentArray) {
	...
	if (embeddedRolimonsUserLink) {
		linkIcon = '<a style="margin-left:5px;margin-top:5px;" target = "_blank" href = "https://www.rolimons.com/player/' + userid + '"><svg id = "roliLink" aria-hidden="true" width="1em" height="1em" style="vertical-align: -0.125em;-ms-transform: rotate(360deg); -webkit-transform: rotate(360deg); transform: rotate(360deg);" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path d="M10.586 13.414a1 1 0 0 1-1.414 1.414 5 5 0 0 1 0-7.07l3.535-3.536a5 5 0 0 1 7.071 7.071l-1.485 1.486a7.017 7.017 0 0 0-.405-2.424l.476-.476a3 3 0 1 0-4.243-4.243l-3.535 3.536a3 3 0 0 0 0 4.242zm2.828-4.242a1 1 0 0 1 1.414 0 5 5 0 0 1 0 7.07l-3.535 3.536a5 5 0 0 1-7.071-7.07l1.485-1.486c-.008.82.127 1.641.405 2.423l-.476.476a3 3 0 1 0 4.243 4.243l3.535-3.536a3 3 0 0 0 0-4.242 1 1 0 0 1 0-1.414z" fill="#fff"></path></svg></a>'
	} else {
		linkIcon = ''
	}
	commentHTML = `                <div class="comment-item list-item" data-comment-id="">
						<div class="comment-user list-header">
							<div class="Avatar avatar avatar-headshot-md roblox-avatar-image" data-user-id="${userid}" data-image-size="small"><div style="position: relative;"><a href="https://www.roblox.com/users/${userid}/profile"><img title="${username}" alt="${username}" border="0" src="https://www.roblox.com/bust-thumbnail/image?userId=${userid}&width=420&height=420&format=png"></a></div></div>
						</div>
						<div class="comment-body list-body">
							<a class="text-name" href="https://www.roblox.com/users/${userid}/profile">${stripTags(username)}</a>${linkIcon}
							<p class="comment-content list-content">${stripTags(comment)}</p>
							<span class="xsmall text-date-hint">${formatDate(date)}</span>
						</div>
						<a userid="${userid}" id="${rand}" class="open-trades-comment" style="display:none;" target="_blank" href="https://www.roblox.com/users/${userid}/trade">
...
function getComments(itemID, page) {
	return new Promise(resolve => {
		chrome.runtime.sendMessage({greeting: "GetURL", url:"https://ropro.io/api/getComments.php?id=" + itemID + "&page=" + page}, 
			function(data) {
				resolve(data)
			}
		)
...
async function refreshComments() {
	...
	seeMore.setAttribute("itemid", `${itemID}`)
	...
	if (commentsJSON.comments.length > 0) {
		...
		document.getElementById('commentsmain').innerHTML += "No comments yet... Be the first to write one!"
...
async function postComment() {
	...
	if (message.value.length != 0) {
		if (message.value.length <= 200) {
			...
			return new Promise(resolve => {
				return new Promise(resolve => {
					chrome.runtime.sendMessage({greeting: "PostURL", url:"https://ropro.io/api/postComment.php", jsonData: json}, 
						function(data) {
