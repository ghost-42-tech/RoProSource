...
function fetchItem(itemID) {
	return new Promise(resolve => {
		chrome.runtime.sendMessage({greeting: "GetURL", url:"https://ropro.io/api/tradeBackend.php?ids=" + itemID}, 
			function(data) {
				resolve(data)
			}
		)
...
function addLink(itemID) {
	...
	if (header != undefined) {
		...
		a.setAttribute('href', 'https://www.rolimons.com/item/' + itemID)
		header.appendChild(a)
	}
}

function projectedDisplay() {
	assetThumbnail = document.getElementById('AssetThumbnail')
...
function addValue(itemValue, itemDemand, itemID) {
	...
	if (details != undefined) {
		...
		demandDiv.innerHTML += '<div class="font-header-1 text-subheader text-label text-overflow field-label"><img src="https://ropro.io/images/ropro_icon_small.png" width="15px"> Demand</div><div class="field-content"><a class="text-name item-genre wait-for-i18n-format-render" target="_blank" href="https://rolimons.com/item/' + itemID + '">' + itemDemand + '</a><span class="wait-for-i18n-format-render"></span></div>'
